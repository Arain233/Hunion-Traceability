<template>
  <div class="createPost-container">
    <el-form ref="postForm" :model="postForm" :rules="rules" class="form-container">
      ...
      <el-steps :active="4" align-center class="showProcess">
        <el-step title="原料商" description="已录入"></el-step>
        <el-step title="生产方" description="已录入"></el-step>
        <el-step title="经销商" description="已录入"></el-step>
        <el-step title="零售商" icon="el-icon-edit" description="正在录入信息..."></el-step>
      </el-steps>
      ...
      <div class="createPost-main-container">
        <el-row>
          <el-col :span="24">
            ...
            <el-form-item style="margin-bottom: 10px;" prop="dealer_code">
              <MDinput v-model="postForm.dealer_code" :maxlength="100" name="dealer_code" required>
                经销商企业代码
              </MDinput>
            </el-form-item>
  ...
  data() {
    const validateRequire = (rule, value, callback) => {
      if (value === '') {
        ...
        callback(new Error(rule.name + '必须填写'))
      } else {
        callback()
      }
    }
    return {
      ...
      status: false,
      rules: {
        Name: [{validator: validateRequire, name: "零售商名称"}],
        dealer_code: [{validator: validateRequire, name: "经销商企业代码"}],
        EnterpCode: [{validator: validateRequire, name: "零售商企业代码"}],
        Address: [{validator: validateRequire, name: "零售商地址"}],
        Amount: [{validator: validateRequire, name: "药材入库数量"}],
        purch_time: [{validator: validateRequire, name: "购买时间"}]
      },
      tempRoute: {}
    }
  },
    ...
    submitForm() {
      ...
      this.$refs.postForm.validate(valid => {
        if (valid) {
          ...
          axios({
            method: 'post',
            url: 'http://120.76.197.109:8282/api/retailer/save',
            data: {
              // "name": this.postForm.name, // 零售商名称
              // "dealer_code": this.postForm.dealer_code,
